...
sell_prices = []
makaslar = []
my_url = "https://altin.doviz.com/gram-altin"
headers = {
        ...
        for satır in satırlar:
            ...
            for bölme in bölmeler:
                if not bölme == bölmeler[0]:
                    ...
                    if bank_name == "placeholder":
                        bank_name = "kapalıçarşı"
...
def bankanın_verisi(banka):
    ...
    global my_url
    my_url_ = f"https://altin.doviz.com/{banka}/gram-altin"
    my_request = requests.get(my_url,headers=headers)
    my_html = BS(my_request.text,'html.parser')
...
if __name__ == "__main__":
    while True:
        ...
        elif process == "Atakip":altın_alış_takip()
        elif process == "Stakip":altın_satış_takip()
